...
public class NewsTopicsJob extends Job {
    ...
    public void onRun() throws Throwable {
        ...
        NewsTopicListParcel newsTopicParcel = IonMethods.getNewsTopicLists(admNo, FirebaseInstanceId.getInstance().getToken());
        if (newsTopicParcel.getError()) {
            EventBus.getDefault().post(new NewsTopicEvent(newsTopicParcel.getResult()));
        } else {
            final List<NewsTopicListParcel.NewsTopics> topics = newsTopicParcel.getTopics();
